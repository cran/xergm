...
\title{Estimating Temporal Exponential Random Graph Models by Bootstrapped Pseudolikelihood\\ \bigskip \Large \texttt{R} package vignette for \pkg{xergm} 1.0}
...
Estimates and 95% confidence intervals:
                         Estimate     2.5%    97.5%
edges                    -9.17591 -10.4516  -8.6876
mutual                    2.96172   2.4657   3.7487
ttriple                   0.21316   0.1274   0.3343
transitiveties            0.36794   0.2223   0.4270
ctriple                  -0.67677  -0.9357  -0.5076
nodeicov.idegsqrt         1.17509   0.9975   1.4825
nodeicov.odegsqrt        -0.28432  -0.5840  -0.1844
nodeocov.odegsqrt         1.19846   1.1637   1.4470
nodeofactor.sex.2         0.60720   0.4228   0.8283
nodeifactor.sex.2         0.22578   0.0732   0.3629
nodematch.sex             1.76805   1.5660   2.1493
edgecov.primary.cov[[i]]  1.05123   0.7605   1.3889
\end{verbatim}
...
The effect is significant because $0$ is outside the confidence interval of $[0.76; 1.39]$.
...
We therefore want the ability to control for friendship choices at each previous time step as well as edge stability (``inertia'', or ``memory'').
...
This would be a memory term in which $1$s and $0$s have the same effect.
Alternatively, one can include separate indicators for edge innovation (the creation of a new edge from $t-1$ to $t$) and edge loss (the dissolution of an edge from $t-1$ to $t$) in order to recover separate coefficients for these two processes.
This may be useful when one or the other (or both) are of specific theoretical interest.
Lastly, a memory term that captures absolute stability, one coded $1$ any time $i$ and $j$ have the same value at time $t-1$ and $t$ (be that value $1$ for a connection or $0$ for a non-connection) may be useful for modeling sparse networks, where connection is a fairly rare event.
...
We can also model dyadic stability using a memory term.
A memory term is a matrix which contains cell entries of $1$ where the current and previous matrix have the same value (i.\,e., in both cases, there is a tie, or in both cases, there is no tie) and otherwise $0$.
We can generate a memory term by specifying \code{lag = TRUE}, \code{covariate = TRUE}, and additionally \code{memory = TRUE} in the \code{preprocess} function.
...
----------------------------------------------------------
edges                       -9.18 *          -9.56 *      
                          [-10.45; -8.69]  [-12.94; -8.51]
mutual                       2.96 *           2.34 *      
                          [  2.47;  3.75]  [  1.84;  2.69]
ttriple                      0.21 *           0.09        
                          [  0.13;  0.33]  [ -0.01;  0.15]
transitiveties               0.37 *           0.43 *      
                          [  0.22;  0.43]  [  0.36;  0.53]
ctriple                     -0.68 *          -0.58 *      
                          [ -0.94; -0.51]  [ -0.71; -0.48]
nodeicov.idegsqrt            1.18 *           1.24 *      
                          [  1.00;  1.48]  [  1.11;  1.60]
nodeicov.odegsqrt           -0.28 *          -0.08        
                          [ -0.58; -0.18]  [ -0.26;  0.18]
nodeocov.odegsqrt            1.20 *           1.66 *      
                          [  1.16;  1.45]  [  1.49;  2.45]
nodeofactor.sex.2            0.61 *           0.62 *      
                          [  0.42;  0.83]  [  0.51;  0.86]
nodeifactor.sex.2            0.23 *           0.38 *      
                          [  0.07;  0.36]  [  0.31;  0.60]
nodematch.sex                1.77 *           1.48 *      
                          [  1.57;  2.15]  [  1.37;  1.83]
edgecov.primary.cov[[i]]     1.05 *           0.40 *      
                          [  0.76;  1.39]  [  0.18;  1.15]
edgecov.delrecip[[i]]                         0.69 *      
                                           [  0.24;  1.59]
edgecov.lag[[i]]                              1.47 *      
                                           [  1.17;  2.48]
edgecov.mem[[i]]                             -1.77 *      
